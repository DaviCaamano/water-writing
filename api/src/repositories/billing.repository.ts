...
export function insertFromInvoice(
  q: Queryable,
  userId: string,
  planType: string,
  isYearPlan: boolean,
...
export function findByInvoiceId(q: Queryable, invoiceId: string) {
  return q.query<{ billing_id: string }>(
    'SELECT billing_id FROM billing WHERE stripe_invoice_id = $1 LIMIT 1',
    [invoiceId],
  );
}

export function getHistory(q: Queryable, userId: string) {
  return q.query<BillingRow>(
    `SELECT * FROM billing
