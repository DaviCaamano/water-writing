...
function Input({ className, type, ...props }: React.ComponentProps<'input'>) {
  return (
    <input
      ...
      data-slot='input'
      className={cn(
        'h-8 w-full min-w-0 rounded-lg border border-border bg-temp',
        'px-2.5 py-1 text-base transition-colors outline-none md:text-sm',
        'file:inline-flex file:h-6 file:border-0 file:bg-temp',
        'file:text-sm file:font-medium file:text-foreground',
        'placeholder:text-black',
        'focus-visible:border-border focus-visible:ring-3 focus-visible:ring-ring/50',
        'disabled:pointer-events-none disabled:cursor-not-allowed',
        'disabled:bg-temp/50 disabled:opacity-50',
